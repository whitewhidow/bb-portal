...
#include "led.h"
#include "captive.h"   // captiveChannel() for the live SoftAP channel in status
...
// Link RSSI (dBm) of the phone connection, read peripheral-side (Web Bluetooth can't
// read connection RSSI on the phone). 0 = not connected / unavailable.
int bleRssi() {
...
static void handleCmd(const char* cmd) {
  ...
  if (!strcmp(cmd, "__VER__")) {
    ...
    char b[160]; snprintf(b, sizeof(b), "st:ble=1:wifi=%d:batt=%d:rssi=%d:up=%lu:heap=%lu:rst=%d:fs=%lu/%lu:cli=%d:ch=%d", netConfigured()?1:0, batteryPct(), bleRssi(), (unsigned long)(millis()/1000), (unsigned long)ESP.getFreeHeap(), (int)esp_reset_reason(), (unsigned long)(LittleFS.totalBytes()-LittleFS.usedBytes()), (unsigned long)LittleFS.totalBytes(), (int)WiFi.softAPgetStationNum(), captiveChannel()); bleNotify(b);
    ...
    // The full cfgJson can exceed the negotiated BLE MTU (a too-big notify silently
    // fails on the C5), so serve it in small pieces like the doc loader:
    // "__CFGGET__[:off]" -> "cfg:<end>:<total>:<chunk-of-the-[...]-body>".
...
void bleTick() {
  ...
  if (g_connected) {
    ...
    if (w != lastW || b/5 != lastB/5 || millis() - lastPush > 5000) {   // 5s keeps RSSI live
      ...
      char m[160]; snprintf(m, sizeof(m), "st:ble=1:wifi=%d:batt=%d:rssi=%d:up=%lu:heap=%lu:rst=%d:fs=%lu/%lu:cli=%d:ch=%d", w, b, bleRssi(), (unsigned long)(millis()/1000), (unsigned long)ESP.getFreeHeap(), (int)esp_reset_reason(), (unsigned long)(LittleFS.totalBytes()-LittleFS.usedBytes()), (unsigned long)LittleFS.totalBytes(), (int)WiFi.softAPgetStationNum(), captiveChannel()); bleNotify(m);
    }
